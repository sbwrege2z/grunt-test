...
function commitInternal(task, options) {
  asyncTask(task, options, commitAsync);
}

function commit(task, options) {
  grunt.task.run(['jshint', 'newer:prettier:all', 'git:commitInternal']);
}
...
function status(task, options) {
  ...
  if (result.untracked || result.merging) {
    match = info.match(
      /Untracked files:\s+\(use "git add <file>\.\.\." to (include in what will be committed|mark resolution)\)\n\n((\t[^\n]+\n)+)/
    );
    if (!match) grunt.fail.fatal('could not find untracked/merging files');
    let rawFiles = match[2];
...
function mergeAsync(task, options) {
  return new Promise(function(resolve, reject) {
    ...
    info = simpleCheckout(task, { branch: branchFrom });
