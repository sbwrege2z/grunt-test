...
function status(task, options) {
  ...
  if (result.untracked || result.merging) {
    match = info.match(
      /(Unmerged paths|Untracked files):(\s+\(use "git reset HEAD <file>\.\.\." to unstage\))?\s+\(use "git add <file>\.\.\." to (include in what will be committed|mark resolution)\)\n\n((\t[^\n]+\n)+)/
    );
    if (!match) grunt.fail.fatal('could not find untracked/merging files');
    let rawFiles = match[4];
